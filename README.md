...
#### Go语法相关
在go语法方面遇到的问题还算小
1. 将\[\]byte转化为string类型时,使用`string(\[\]byte)`报错,后使用`string(\[\]byte\[:\])`后不报错,但是在数据存入数据库中时,数据库报错,经检验,转化后string出现乱码,无法存储在数据库中。使用`string = fmt.Sprint("%x",\[\]byte)`后得以解决。
2. 切片理解上产生问题,目前没有使用,未解决。
3. 包中自定义函数需首字母大写,才可以在包外被使用。

#### gin与gorm
1. 使用edge浏览器访问网站,无法写入cookie,换个浏览器得以解决。

#### 网页部署
网页部署过,是我这次任务遇到问题最多的环节,我几度想要放弃。、
1. 腾讯云无法直接使用图形化界面linux,使用vnc速度太慢,最后被迫接受了命令行操作界面。
2. 使用edge浏览器,在腾讯云网页远程安装软件时,网页报错,重新登陆后,系统安装软件报错,需要重装系统,换个浏览器没有问题。
3. 服务器安装mysql时初始化密码时出错,重装了其他版本的1mysql。
4. 服务器80端口经常被占用,需要频繁更改端口。使用nginx反向代理,得以解决。(虽然只增加了一行)
5. 使用vim遇到了极大的困难,被迫学习,但比vi好用一点。
6. 在将文件部署到linux服务器后,go版本不匹配,试图安装对应版本的go,但是官网下载速度太慢,镜像网站没有新版本的go。最后,重新`go mod init` , `go mod tidy` 解决。
7. 服务器不知道怎么给git加速,最后手动部署。
